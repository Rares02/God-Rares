using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public static class UIText
{
    public static readonly string INFO_PANEL_CELL_EMPTY = "Empty";
    public static readonly string INFO_PANEL_LEVEL = "Level";
    public static readonly string INFO_PANEL_TIME = "Time";
    public static readonly string INFO_PANEL_MONEY = "Money";
    public static readonly string INFO_PANEL_FAITH = "Faith";
    public static readonly string INFO_PANEL_FOLLOWERS = "Followers";
    public static readonly string INFO_PANEL_MONEY_CAPACITY = "Money Capacity";
    public static readonly string INFO_PANEL_FAITH_CAPACITY = "Faith Capacity";
    public static readonly string INFO_PANEL_FOLLOWERS_CAPACITY = "Followers Capacity";
}
